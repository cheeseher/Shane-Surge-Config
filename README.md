...
- [Surge 代理协议与 UDP Relay 支持范围](https://manual.nssurge.com/policy/proxy.html)
- [OpenAI 网络建议与 ChatGPT Voice 端口](https://help.openai.com/en/articles/9247338-network-recommendations-for-chatgpt-errors-on-web-and-apps)
...
- [Amy AnyTLS 与关闭 TFO 公告](https://www.amytele.com/)
...
| `Zoom会议` | Zoom 登录、语音、视频、共享屏幕和直接 IP 媒体连接 | 中国大陆先用 `香港通话` |
...
- `AI 连通性监测`：检查 ChatGPT、Claude、Grok、Gemini、GitHub，并显示延迟、出口地区、最终实际节点和设备时钟。
...
3. `UDP + TCP` 或 `UDP 活跃` 表示已观察到 UDP 媒体路径；`TCP 回退` 表示当前只观察到 TCP，不等于已经断线，但在丢包时语音恢复能力通常弱于 UDP；`等待通话数据` 不是失败，只表示近期连接里没有足够样本。
4. 同时核对最终节点：Telegram 通常应为 `新加坡通话 › 某条新加坡节点`；Zoom 在中国大陆应为 `香港通话 › 某条香港节点`。

该模块使用 Surge 官方 HTTP API 读取当前与近期连接，只分析真实流量，不伪造 UDP 测试，也不能代替双方开麦、静音、锁屏、共享屏幕的完整验收。Amy 当前全线 AnyTLS，而 Surge 官方 UDP Relay 支持列表未列出 AnyTLS，因此出现 `TCP 回退` 是可预期现象；若持续单向无声或卡顿，需要换香港/新加坡实际节点并按后文验收，而不是把 Zoom 改回分路。
...
- `Zoom会议` 默认 `香港通话`。在中国大陆，直连可能可以入会，却无法开启或稳定维持共享屏幕，因此不再把 `DIRECT` 作为默认。
...
- 香港线路不稳时，将整组切到 `新加坡通话`，退出并重新加入会议；`DIRECT` 只用于临时诊断，不是大陆推荐项。

Zoom 官方指定 Meetings/Webinars 主要使用 TCP 443/8801-8802 和 UDP 3478-3479/8801-8810，并明确 UDP 3478 用于 TURN。本配置已使用官方当前 IP 范围覆盖直接 IP 媒体连接。Amy 的 AnyTLS 线路可能让 Zoom 通过 TCP 443 回退完成入会与共享屏幕，但这不能等同于 UDP 媒体质量保证。

### Zoom Mac 会前设置

1. Surge 同时开启 `Set as System Proxy` 和 `Enhanced Mode`。
2. `Zoom会议` 保持 `香港通话`，并确认已落到一条真实香港节点；关闭其他 VPN、代理与可能接管网络的安全软件。
...
如香港线路超过门槛或出现稳定复现的单向无声，将整个 `Zoom会议` 切到 `新加坡通话`，退出后重新加入会议并完整复测。不要只代理 UDP、只代理投屏，或在会议中频繁切换节点。
...
| 香港线路数据差 | 整个组切到新加坡通话后重新入会并复测 |
| 香港、新加坡都持续卡顿 | 逐条更换实际节点并复测；`DIRECT` 仅用于判断是否为代理路径问题，不作为大陆共享屏幕方案 |
...
| --- | --- | --- |
| 节点选择 | 香港节点 | 大陆日常延迟优先 |
| 人工智能 | AI美国固定 | ChatGPT / Claude / Grok / Copilot 共用一个真实节点 |
| 谷歌服务 | AI日本固定 | Google / Gemini 共用一个真实节点 |
| 电报消息 | 新加坡通话 | 单实际节点，小波动不换线 |
| 国际媒体 | 美国节点 | 优先美区内容 |
| 领英 | 香港真实节点 | 登录期间固定出口，避免账户 IP 频繁变化 |
| TikTok | TikTok日本固定 | 进入子组选择一条真实日本节点并长期保持 |
| Zoom会议 | 香港通话 | 大陆共享屏幕所需；整个会议同一代理出口 |
...
- Amy 官网已公告全线切换 AnyTLS，并要求客户端更新后关闭 TFO；配置已通过 `external-policy-modifier="tfo=false"` 对订阅节点强制执行。
- Surge 官方文档当前未把 AnyTLS 列入 UDP Relay 支持列表，因此 Telegram/Zoom 可能显示 TCP 回退；这不是规则能够补齐的节点能力。
- 泰国网络直连拉取订阅返回 403，仍无法在当前网络完成 Amy 实际节点连通与会议测试。
...
`scripts/update_official_network_rules.mjs` 会读取 OpenAI ChatGPT Voice 与 Zoom Meetings 官方发布的 IPv4/IPv6 范围，只更新配置内的受控标记区。GitHub Actions 每周运行一次；有变化时创建审查 PR，不会绕过人工检查直接改 `main`。
